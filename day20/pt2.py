s = """p=<-3770,-455,1749>, v=<-4,-77,53>, a=<11,7,-9>
...
class Particle:
    def __init__(self, index, description):
        self.index = index
        m = re.match(r"p=<(-?\d+),(-?\d+),(-?\d+)>, v=<(-?\d+),(-?\d+),(-?\d+)>, a=<(-?\d+),(-?\d+),(-?\d+)>", description)
    ...
    def update(self):
        self.velocity = tuple_add(self.velocity, self.acceleration)
        self.position = tuple_add(self.position, self.velocity)

    def manhattan(self):
        return sum(map(abs, self.position))

particles = [Particle(index, line) for index, line in enumerate(s.splitlines())]

def show_closest():
    for particle in islice(sorted(particles, key=lambda p: p.manhattan()), 5):
        print(particle.index, particle.manhattan())
...
for n in range(100):
    for t in range(1000):
        ...
        for particle in particles:
            particle.update()
            if particle.position in positions:
                collisions = collisions.union({ positions[particle.position], particle.index })
            else:
                positions[particle.position] = particle.index
        particles = list(filter(lambda p: p.index not in collisions, particles))
    print(len(particles))

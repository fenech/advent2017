...
def read_instruction(line):
    ...
    for group in (1, 4):
        if m.group(group) not in registers:
            registers[m.group(group)] = 0

    if eval("registers['{}'] {}".format(m.group(4), m.group(5))):
        registers[m.group(1)] += increment

    return max(val for key, val in registers.items())

print(max(read_instruction(line) for line in s.splitlines()))
